...
test_that("gt_snmf error messages", {
  # wrong file input
  expect_error(gt_snmf(
    x = pops_path, project = "force", k = 1:10, entropy = TRUE,
    percentage = 0.5, n_runs = 1, seed = 1, alpha = 100
  ), "The input file must be a .geno")
  # file that doesn't exist
  invalid_geno <- paste0(tempfile(), ".geno")
  expect_error(gt_snmf(
    x = invalid_geno, project = "force", k = 1:10, entropy = TRUE,
    percentage = 0.5, n_runs = 1, seed = 1, alpha = 100
  ), "does not exist")
  # random object
  expect_error(gt_snmf(
    x = 1, project = "force", k = 1:10, entropy = TRUE,
    percentage = 0.5, n_runs = 1, seed = 1, alpha = 100
  ), "x must be a gen_tibble or a character giving the path")
  # seed != n_runs
  expect_error(gt_snmf(
    x = anole_gt, project = "force", k = 1:10, entropy = TRUE,
    percentage = 0.5, n_runs = 1, seed = c(1, 2), alpha = 100
