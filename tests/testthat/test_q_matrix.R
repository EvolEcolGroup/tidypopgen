...
test_that("get_q_matrix returns correct q-matrix", {
  ...
  q_list_no_structure <- list(
    as.matrix(read.table(system.file("/extdata/anolis/anolis_ld_run1.3.Q", package = "tidypopgen"), header = TRUE)),
...
test_that("get_p_matrix returns correct p-matrix", {
  ...
  q_list$P <- list(
    as.matrix(read.table(system.file("/extdata/anolis/anolis_ld_run1.3.P", package = "tidypopgen"), header = TRUE)),
