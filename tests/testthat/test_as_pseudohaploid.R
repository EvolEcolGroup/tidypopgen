...
test_that("as_pseudohaploids correctly deals with ploidy",{
  # confirm that the gen_tibble is diploid for the moment
  expect_equal(show_ploidy(test_gt), 2)
  # now use as_pseudohaploid
  test_gt_pseudo <- as_pseudohaploid(test_gt)
  # now check that ploidy is -2
  expect_equal(show_ploidy(test_gt_pseudo), -2)
  ## TODO check the individual ploidies
  
  ## TODO check that we can reprocess it (use a different number of loci to get different result)
})
