...
test_that("test we can write and read a vcf", {
  ...
  test_gt2 <- gen_tibble(vcf_path, quiet = TRUE,
                         parser = "vcfR", backingfile = tempfile())
  ...
  test_gt2 <- gen_tibble(vcf_path, quiet = TRUE,
                         parser = "cpp", backingfile = tempfile())
...
test_that("test reading and writing is equivalent", {
  ...
  pop_a_vcf <- gen_tibble(vcf_path, quiet = TRUE,
                          backingfile = tempfile(), parser = "vcfR")
  # write it out again
  file <- paste0(tempfile(), ".vcf")
  write_vcf_path <- gt_as_vcf(pop_a_vcf, file = file)
  # reread
  pop_a_vcf_rewrite <- gen_tibble(write_vcf_path,
                                  quiet = TRUE,
                                  backingfile = tempfile(), parser = "vcfR")
  # because of the different backing file info,
  # we cannot use identical on the whole object
  expect_true(identical(show_genotypes(pop_a_vcf),
...
test_that("test reading and writing with chunking is equivalent cpp", {
  ...
  vcf_path_chunked <- gt_as_vcf(test_gt,
                                file = paste0(tempfile(), ".vcf"),
                                chunk_size = 2)
  # read vcf back in with cpp
  test_gt_chunked <- gen_tibble(vcf_path_chunked,
                                quiet = TRUE,
                                parser = "cpp",
                                backingfile = tempfile("anolis_chunk_"))
  # check they are the same
  expect_true(identical(show_genotypes(test_gt),
