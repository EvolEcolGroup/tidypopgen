...
test_that("pop_* basic stats functions work correctly", {
  ...
  expect_error(
    test_gt %>% pop_fis(by_locus = TRUE, method = "Nei87", allele_sharing_mat = matrix(1, nrow = 7, ncol = 7)),
    "allele_sharing_mat not relevant for Nei87"
  )
  # test Fis errors for incorrect parameters for WG17
  expect_error(
    test_gt %>% pop_fis(by_locus = TRUE, method = "WG17", allele_sharing_mat = matrix(1, nrow = 7, ncol = 7)),
