...
test_that("create gen_tibble from dfs", {
  test_dfs_gt <- gen_tibble(test_genotypes,
...
test_that("gen_tibble catches invalid alleles", {
  ...
  expect_error(test_dfs_gt <- gen_tibble(test_genotypes,
    indiv_meta = test_indiv_meta,
...
test_that("if order of loci is changed, order of genotypes also changes", {
  pop_b <-
    gen_tibble(system.file("extdata/pop_b.bed", package = "tidypopgen"),
...
test_that("gen_tibble does not accept character matrix", {
  ...
  expect_error(test_dfs_gt <- gen_tibble(test_genotypes_c,
    indiv_meta = test_indiv_meta,
    loci = test_loci, quiet = TRUE
...
test_that("gen_tibble wrong filetype error", {
  expect_error(
    test_dfs_gt <-
      gen_tibble(system.file("extdata/related/test_king.kin0",
        package = "tidypopgen"
      )),
...
test_that("gen_tibble loci is dataframe or tbl", {
  ...
  expect_error(test_dfs_gt <- gen_tibble(test_genotypes,
    indiv_meta = test_indiv_meta,
    loci = wrong_loci_matrix, quiet = TRUE
...
test_that("gen_tibble identifies wrong dimensions in genotypes", {
  ...
  expect_error(
    test_dfs_gt <- gen_tibble(wrong_genotypes,
      indiv_meta = test_indiv_meta,
      loci = test_loci, quiet = TRUE
    ),
    paste(
...
test_that("PROBLEM TEST gentibble from VCF with missingness issue", {
  ...
  # reload it in chunks
  pop_b_vcf_gt2 <- gen_tibble(vcf_path,
    quiet = TRUE, backingfile = tempfile(),
  ...
  pop_b_vcf_fast_gt <-
    gen_tibble(vcf_path, quiet = TRUE, backingfile = tempfile(), parser = "cpp")
  ...
  # reload it in chunks
  pop_b_vcf_fast_gt2 <- gen_tibble(vcf_path,
    quiet = TRUE, backingfile = tempfile(),
...
test_that("gen_tibble from files", {
  ...
  raw_file_pop_a <-
    read.table(system.file("extdata/pop_a.raw", package = "tidypopgen"),
      header = TRUE
    )
  ...
  pop_a_vcf_gt <-
    gen_tibble(vcf_path,
      quiet = TRUE,
      backingfile = tempfile(), parser = "vcfR"
    )
  expect_true(all.equal(
  ...
  # reload it in chunks
  pop_a_vcf_gt2 <- gen_tibble(vcf_path,
    quiet = TRUE, backingfile = tempfile(),
    chunk_size = 2, parser = "vcfR"
  )
  expect_true(all.equal(
  ...
  pop_a_vcf_fast_gt <-
    gen_tibble(vcf_path,
      quiet = TRUE,
      backingfile = tempfile(), parser = "cpp"
    )
  expect_true(all.equal(
    show_genotypes(pop_a_gt),
  ...
  # reload it in chunks
  pop_a_vcf_fast_gt2 <- gen_tibble(vcf_path,
    quiet = TRUE, backingfile = tempfile(),
    chunk_size = 2, parser = "cpp"
  )
  expect_true(all.equal(
...
test_that("gen_tibble from files with missingness", {
  ...
  # now read the dosages created by plink when saving in raw format
  raw_file_pop_b <-
    read.table(system.file("extdata/pop_b.raw", package = "tidypopgen"),
      header = TRUE
    )
...
test_that("gentibble with packedancestry", {
  ...
  pop_a_gt <-
    gen_tibble(geno_path,
      quiet = TRUE,
      backingfile = tempfile(), valid_alleles = c("A", "G", "C", "T")
    )
  ...
  # swap around the dosages in .raw matrix to check correspondence with
  # packedancestry genotypes
  ...
  # for these two, the gen_tibble from ped and from geno should match
  ...
  expect_equal(
    show_loci(pop_a_gt_ped_swap)$allele_ref,
    show_loci(pop_a_gt_swap)$allele_ref
  )
  expect_equal(
    show_loci(pop_a_gt_ped_swap)$allele_alt,
    show_loci(pop_a_gt_swap)$allele_alt
  )
  ...
  expect_equal(
    show_loci(pop_a_gt_ped_snp)$allele_ref,
    show_loci(pop_a_gt_snp)$allele_alt
  )
  expect_equal(
    show_loci(pop_a_gt_ped_snp)$allele_alt,
    show_loci(pop_a_gt_snp)$allele_ref
  )
  ...
  # swap around the dosages in .ped_gtypes matrix to check correspondence
  # with packedancestry genotypes
...
test_that("gentibble with packedancestry and missingness", {
  ...
  # read in ped to check loci
  ped_path <- system.file("extdata/pop_b.ped", package = "tidypopgen")
  pop_b_gt_ped <- gen_tibble(ped_path,
    quiet = TRUE, backingfile = tempfile(),
    valid_alleles = c("A", "G", "C", "T")
  )
  ...
  # for these two, the gen_tibble from ped and gen_tibble from geno should
  # therefore match
  ...
  # check loci and genotypes
  expect_equal(
    show_loci(pop_b_gt_ped_swap)$allele_ref,
    show_loci(pop_b_gt_swap)$allele_ref
  )
  expect_equal(
    show_loci(pop_b_gt_ped_swap)$allele_alt,
    show_loci(pop_b_gt_swap)$allele_alt
  )
  ...
  # check loci and genotypes
  expect_equal(
    show_loci(pop_b_gt_ped_snp)$allele_ref,
    show_loci(pop_b_gt_snp)$allele_alt
  )
  expect_equal(
    show_loci(pop_b_gt_ped_snp)$allele_alt,
    show_loci(pop_b_gt_snp)$allele_ref
  )
  ...
  # swap around the dosages in .ped_gtypes matrix to check
  # correspondence with packedancestry genotypes
  ped_gtypes[zero_positions] <- -1
  ped_gtypes[two_positions] <- 0
  ped_gtypes[ped_gtypes == -1] <- 2
...
test_that("chr_int is always an integer", {
  ...
  pop_a_vcf_gt <-
    gen_tibble(vcf_path,
      quiet = TRUE, backingfile = tempfile(), parser = "vcfR"
    )
  expect_true(is.integer(show_loci(pop_a_vcf_gt)$chr_int))
  pop_a_vcf_fast_gt <-
    gen_tibble(vcf_path,
      quiet = TRUE, backingfile = tempfile(), parser = "cpp"
    )
  expect_true(is.integer(show_loci(pop_a_vcf_fast_gt)$chr_int))
...
test_that("additional vcf tests with larger file", {
  ...
  anole_gt <- gen_tibble(vcf_path,
    quiet = TRUE,
    parser = "cpp", backingfile = tempfile("anolis_")
  )
  anole_gt_vcfr <- gen_tibble(vcf_path,
    quiet = TRUE, parser = "vcfR",
  ...
  anole_gt2 <- gen_tibble(vcf_path,
    quiet = TRUE, parser = "cpp", backingfile = tempfile("anolis_"),
...
test_that("vcf's with haploid markers", {
  ...
  vcf_path_haploid <- system.file("extdata/haploid_first_pop_a.vcf",
    package = "tidypopgen"
  )
  # the cpp parser catches the problem
  expect_error(
    pop_a_vcf_gt_hap_cpp <-
      gen_tibble(vcf_path_haploid,
        quiet = TRUE,
        backingfile = tempfile(), parser = "cpp"
      ),
    "a genotype"
  ...
  pop_a_vcf_gt_hapmid_cpp <-
    gen_tibble(vcf_path_haploid_middle,
      quiet = TRUE,
      backingfile = tempfile(), parser = "cpp"
    )
