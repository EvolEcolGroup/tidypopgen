...
loci_missingness.tbl_df <- function(.x,
  ...
  loci_missingness(.x$genotypes, as_counts = as_counts, block_size = block_size, ...)
}
...
#  stopifnot_diploid(.x)
  ...
  if (length(rows_to_keep)>1){
    ...
    count_na_sub<- function(BM, ind, rows_to_keep){
      n_na <- bigstatsr::big_counts(BM, ind.row = rows_to_keep,
                                    ind.col = ind)
      n_na <- n_na[nrow(n_na),] # this should work also with polyploids
      n_na
    }
    n_na <-  bigstatsr::big_apply(geno_fbm, a.FUN = count_na_sub,
...
loci_missingness.grouped_df <- function(.x, as_counts = FALSE,
    ...
    count_na_sub<- function(geno_fbm, ind, rows_to_keep){
      na_mat <- gt_grouped_missingness(BM = geno_fbm,
                                            rowInd = rows_to_keep,
                                            colInd = ind,
    ...
    na_mat <- bigstatsr::big_apply(geno_fbm, a.FUN = count_na_sub,
                         rows_to_keep = rows_to_keep,
