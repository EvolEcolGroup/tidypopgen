...
tidy.gt_pca <- function(x, matrix = "u", ...) {
  ...
  if (matrix %in% c("pcs", "d", "eigenvalues")) {
    ret <- tibble(PC = seq_len(length(x$eig)),
                  "std.dev" = x$eig) %>%
      mutate(percent = .data$std.dev/sum(.data$std.dev),
...
#' @param drop_genotypes boolean on whether genotypes should be droppved from
#' the tibble being returned by the function. As the `genotypes` column has
...
#' @param ... Not used. Needed to match generic signature only.
#' @return A [tibble::tibble] (or a [gen_tibble] if `drop_genotypes` is set
...
augment.gt_pca <- function(x, data = NULL, drop_genotypes = TRUE, ...) {
  if (!is.null(data) & drop_genotypes){
    ...
    ret <- if (!missing(data) && !is.null(data)) {
      ...
      tibble(.rownames = rownames(as.data.frame(x$scores))) %>%
        add_column(pred)
    }
    ret
}
