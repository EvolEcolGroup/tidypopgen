...
#' @param group a vector of group memberships to order the individuals
#' in "components" plot. If NULL, the
#' clusters used for the DAPC will be used.
                             ...
                             ld = NULL, group = NULL, n_col = 1, ...)
{
  ...
  if (type== "screeplot") {
    ...
    #stop("autoplot for gt_dapc does not have a loading option yet")
    if (is.null(object$var.load)){
      stop("the dapc object was saved without loadings for loci, rerun it with 'loadings_by_locus = TRUE'")
    }
    if (is.null(ld)){
      ld <- 1
    }
    if (length(ld) > 1) {
      ...
      p <- ggplot2::ggplot(mapping = ggplot2::aes(x = bigstatsr::rows_along(object$var.load),
                                                  y = object$var.load[, ld])) +
        ggplot2::geom_point() +
        bigstatsr::theme_bigstatsr(size.rel = 1) +
        ggplot2::labs(title = paste0("Loadings of LD", ld),
                      x = "Locus", y = NULL)
      nval <- nrow(object$var.load)
      `if`(nval > 12, p, p + ggplot2::scale_x_discrete(limits = factor(seq_len(nval))))
    ...
    q_tbl <- object$posterior %>%
      tibble::as_tibble() %>% dplyr::rename_with(~paste0("Q",.x)) %>%
      # add the pops data for plotting
      dplyr::mutate(name = rownames(object$posterior),
             group = group) %>%
      tidyr::pivot_longer(cols = starts_with("Q"), names_to = "q", values_to = "prob")

    ggplot2::ggplot(q_tbl, ggplot2::aes(x = .data$name, y = .data$prob, fill = .data$q)) +
      ggplot2::geom_col(color = "gray", size = 0.1)+
      ggplot2::facet_grid(~group, switch = "x", scales = "free", space = "free")+
