...
loci_hwe.vctrs_bigSNP <- function(.x, ...) {
  ...
  if (length(rows_to_keep)>1){
    ...
    colHWE_sub <- function(X, ind, rows_to_keep) {
      ...
      apply(X[rows_to_keep, ind], 2, SNPHWE_vec)
    }
...
SNPHWE_vec <- function(x){
  SNPHWE(sum(x==0, na.rm = TRUE),
         sum(x==1, na.rm = TRUE),
         sum(x==2, na.rm = TRUE))
}
...
# code from https://csg.sph.umich.edu/abecasis/Exact/r_instruct.html
# c++ code available from there

SNPHWE <- function(obs_hom1, obs_hets, obs_hom2)
{
