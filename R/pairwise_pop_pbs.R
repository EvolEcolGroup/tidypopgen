...
#' and is used to identify outlier
#' loci that may be under selection.
#' @param .x A grouped `gen_tibble`
#' @param fst_method A character string specifying the method to use for computing
#' Fst
...
pairwise_pop_pbs <- function(.x, fst_method = "Hudson", return_fst = FALSE) {
  ...
  fst_values <- pairwise_pop_fst(.x, method = fst_method, by_locus = TRUE)$Fst_by_locus
  
  # create all 3 way combinations of populations
  pop_combinations <- combn(.group_levels %>% dplyr::pull(1), 3, simplify = FALSE)
  # @TODO the above will only work if we have one grouping level
  
  # for each combination of populations, compute the pbs
  pbs_results <- lapply(pop_combinations, function(pops) {
    pop1 <- pops[1]
    pop2 <- pops[2]
    pop3 <- pops[3]
    
    # Extract Fst values for the current combination
