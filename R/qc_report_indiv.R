...
qc_report_indiv.grouped_df <- function(.x, kings_threshold = NULL, ...){
  ...
  indivs_to_keep <- function(relatives,king){
    for(i in 1:length(king)){
      output <- filter_high_relatedness(king[[i]],kings_threshold = kings_threshold)[[3]]
  ...
  qc_report_indiv <- .x %>% ungroup() %>% reframe(het_obs = indiv_het_obs(.x),
                                                  missingness = indiv_missingness(.x,as_counts=FALSE))
  qc_report_indiv$to_keep <- all_flags
  qc_report_indiv$id <- .x$id
...
autoplot.qc_report_indiv <- function(object, type = c("scatter", "relatedness"),miss_threshold = NULL, kings_threshold = kings_threshold, ...){
  ...
  if (type == "scatter") {
    final_plot <- autoplot_qc_report_indiv(object,miss_threshold = miss_threshold)
  } else if (type == "relatedness") {
    final_plot <- autoplot_qc_report_indiv_king(object,kings_threshold = kings_threshold)
  } else {
