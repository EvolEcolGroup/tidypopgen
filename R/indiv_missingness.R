...
indiv_missingness.tbl_df <- function(.x,
  ...
  indiv_missingness(.x$genotypes, as_counts = as_counts, block_size = block_size, ...)
}
...
indiv_missingness.vctrs_bigSNP <- function(.x,
  ...
  count_row_na_sub <- function(X, ind, rows_to_keep){
    row_counts <- bigstatsr::big_counts(X, ind.col=ind,
  ...
  row_na <-  bigstatsr::big_apply(X, a.FUN = count_row_na_sub,
