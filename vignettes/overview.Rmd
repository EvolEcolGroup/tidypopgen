...
It is also possible to export data into `adegenet` `genelight` objects, which are
...
plink_gen_tbl2 <- read_plink_raw(file = raw_path2, map_file = map_path2)
plink_gen_tbl2
```
...
`rbind_dry_run`:
```{r}
report <- rbind_dry_run(plink_gen_tbl, plink_gen_tbl2, flip_strand = TRUE,
...
```{r}
plink_gen_tbl_new <- rbind_gen_tbl(plink_gen_tbl, plink_gen_tbl2, flip_strand = TRUE,
                        remove_ambiguous = TRUE)

```

```{r}
plink_gen_tbl_new %>% show_loci()
