...
and a vector defining the ploidy of each individual, stored as the attribute 
`ploidy` to the column `genotypes`. 
...
```{r}
test_gen %>% show_genotypes(genotypes)
```
...
```{r}
test_gen %>% ind_H_obs(genotypes)
```

We can use `mutate` to add observed heterozygosity as a column to our `gen_tibble`:
```{r}
...
We can get the frequency of the alternate alleles per population with:
```{r}
test_gen %>% group_by(population) %>% reframe(sums=.genotypes_sums(genotypes))
```
