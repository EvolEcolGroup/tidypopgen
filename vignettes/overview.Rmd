...
Certain metrics and analyiss are naturally defined by a grouped tibble 
...
Alternatively, we can write a plink `ped` file with:
```{r}
gt_write_plink(plink_gt, file = out_file, plink_format = "ped")
```


It is also possible to import data from `adegenet::genelight` objects, which are
well integrated into a number of existing packages to explore the data with, for example, PCA and
clustering, as well as a wealth of export possibilities. Given an object
`x_genlight`, we can simply create a `gen_tibble` object with
`as_gen_tibble(x_genlight)`.
