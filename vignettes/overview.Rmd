...
There are a number of functions that extimate quantities at the *individual* 
...
Certain metrics and analysis are naturally defined by a grouped tibble as they
...
an object in of  the appropriate type; other functions, such as `gt_as_plink()` or `gt_as_geno_lea()`
write a file in the appropriate format, and return the name of that file onc completion.
For example, to export to a PLINK .bed file, we simply use:

```{r}
gt_as_plink(example_gt, bedfile =  tempfile("new_bed_"))
```

This will also write a .bim and .fam file and save them together with the .bed file. 
Note that, from the main tibble, only
on `id`, `population` and `sex` will be preserved in the .fam file.
...
More datails about PCA and other analysis is found in the vignette on population
genetic analysis.
