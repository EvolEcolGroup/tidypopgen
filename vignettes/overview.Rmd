...
We create a plink BED file (we save it into a temp file for this vignette):
```{r}
bed_path <- gt_write_bed_from_dfs(genotypes = example_genotypes, 
                  loci = example_loci, 
                  ind_meta = example_ind_meta, 
                  path_out = file.path(tempdir(),'test_data'))
bed_path
```
...
```{r}
example_gt <- gen_tibble(bed_path)
```
We are provided information on where the two files underlying the genotype information
are stored. As we did use the default options, they are placed in the same directory
as the original .bed file, using the same basename "test_data". It is important that these
...
We can also subset and reorder by passing indeces:
```{r}
...
```{r}
example_gt %>% mutate(het_obs = ind_het_obs(genotypes)) %>% group_by(population) %>% 
...
Currently, `tidypopgen` can read data only from `bigSNP` objects from the `bigsnpr` package. That package has import functions for plink BED files. We will
need to add some more functionality. It would be good to read the bed files directly (easy) and import from vcfR (not too difficult via `genio`, we just
need to block process the data).

`bigSNP` can be exported to BED, we just need a wrapper to subsets individuals
and loci based on the `gen_tbl`. That's easy.
...
snp_file <- snp_readBed(bedfile, backingfile = tempfile("example_"))
...
snp_file <- snp_readBed(bedfile, backingfile = tempfile("missing_"))
missing_gt <- gen_tibble(snp_file)
missing_gt
```
...
The moral of the story is that we need to write a wrapper for a standard PCA which is
less demanding in terms of data!
