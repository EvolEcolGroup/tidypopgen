...
from another csv file. We will have add the population information manually. Let's start by reading the
file:
```{r}
...
Lets begin by visualising our samples geographically. First we need to load the
coordinates of our samples:

```{r}
coord_path <- system.file("/extdata/anolis/anolis_coordinates.csv",
  package = "tidypopgen"
)
coord <- readr::read_csv(coord_path)
coord
```

We can add this information to our gentibble object by using a
`left_join` followed by the function `gt_add_sf()`. The latter function will add
an sf geometry column to the gen_tibble object, which can be used to plot the
samples on a map.

```{r}
anole_gt <- anole_gt %>% left_join(coord, by = "id")
...
continents_map_SA <- ne_countries(country = sa_countries, type = "map_units", scale = "medium") %>%
  st_union()

ggplot() +
  geom_sf(data = continents_map_SA) +
  geom_sf(data = anole_gt$geometry) +
  coord_sf(crs = st_crs(4326),xlim = c(-85,-30), ylim = c(-30,15),expand = FALSE) +
  theme_minimal()
```
...
Atlantic forest lizards appear to form distinct geographic cluster, separate from
