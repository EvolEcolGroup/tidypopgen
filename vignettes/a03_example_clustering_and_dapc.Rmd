...
anole_adm <- read_q_files(dir)

# add attributes to the q matrices
for (i in c(1:5)) {
  attr(anole_adm$Q[[i]], "group") <- anole_gt$population
}

anole_adm$cv <- c(
  0.61043, 0.44932, 0.49835,
  0.47631, 0.52006
)
anole_adm$loglik <- c(
  -82846.88, -58515.56,
  -55713.79, -49499.16, -44040.47
)
anole_adm$algorithm <- "ADMIXTURE"
anole_adm$id <- anole_gt$id

files <- list.files(dir, pattern = "\\.P(?:\\.gz)?$", full.names = TRUE)

matrix_list <-
  lapply(
    files,
    function(file) q_matrix(utils::read.table(file, header = FALSE))
  )
anole_adm$P <- matrix_list
files <- list.files(dir, pattern = "\\.log.gz$", full.names = TRUE)
matrix_list <- lapply(files, function(file) readLines(file))
anole_adm$log <- matrix_list
anole_adm$group <- anole_gt$group
```

Note that within this vignette we are not running the `gt_admixture` function, as it requires the ADMIXTURE software to be installed on your machine and available in your PATH. Instead, we load the results from a previous run of ADMIXTURE.

If you would prefer a native clustering algorithm, we recommend using the `gt_snmf` function, which is a wrapper for the `snmf` function from the `LEA` package. sNMF is a fast clustering algorithm which provides results similar to STRUCTURE and ADMIXTURE, and is available directly using the `tidypopgen` package.
...
plot(anole_adm$cv, cex = 1.2, pch = 19)
```
...
plt <- ggplot2::ggplot(tidy_q, ggplot2::aes(x = id, y = percentage, fill = q)) +
  ...
  ggplot2::labs(
    ...
    title = "Clustering at K = 3 for Anolis punctatus using ADMIXTURE algorithm"
  ) +
  theme_distruct() +
  scale_fill_distruct()
...
```{r}
anole_gt$forest <- ifelse(anole_gt$population == "AF",
  "Atlantic Forest", "Amazonian Forest"
)
```
...
```{r}
anole_adm <- gt_admix_reorder_q(anole_adm, group = anole_gt$forest)
```
...
In the analysis above, through `gt_admxiture()`, we ran 1 repetition for K values 1 to 10. 
