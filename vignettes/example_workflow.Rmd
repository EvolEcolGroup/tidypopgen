...
Using `tidy` on the pca object allows us easily obtain those quantities, and it is then
...
```{r}
anole_clusters <- gt_pca_find_clusters(anole_pca, n.pca = 17)
```
...
This is of class dapc, so we can use the standard `adegenet` functions:
```{r}
anole_dapc
scatter(anole_dapc)
