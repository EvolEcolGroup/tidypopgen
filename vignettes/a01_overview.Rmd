...
all the information stored in the `gen_tibble`. However, several `gen_tibble` can work with the same `.rds` and `.bk` files (for example, if we subset individuals or loci).
...
Occasionally, we may need to update the backing files of a `gen_tibble` (the `.rds` and `.bk` files), as some functions require that the number of individuals in the `gen_tibble` object loaded in the R session matches the number of individuals in the backing files that are stored on disk.
