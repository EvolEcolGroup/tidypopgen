...
all the information stored in the `gen_tibble`. However, several `gen_tibble` objects can work with the same `.rds` and `.bk` files (for example, if we create different subsets of individuals or loci). The schematic below illustrates this.

![Figure 2: Visual representation of updating backingfiles for a gen_tibble object](./img/gt_save.jpg)

The `.rds` and `.bk` file have to share name, but the `.gt` file can be named differently. By default, if
no specific name is given `gt_save` will use the same pattern as the `.rds` 
and `.bk` file, and if that pattern is already in use by another `.gt` object, a version number will be appended.
...
![Figure 2: Visual representation of updating backingfiles for a gen_tibble object](./img/update_backingfile.jpg){width=630px}
...
Let's create another `gen`_tibble`, where loci are not ordered:
