...
all the information stored in the `gen_tibble`. However, several `gen_tibble` objects can work with the same `.rds` and `.bk` files (for example, if we subset individuals or loci).

Because of that, whilst the `.rds` and `.bk` file have to share name, the `.gt` file can be named differently (but, by default, if
no specific name is given, `gt_save` will use the same pattern as for the `.rds` 
and `.bk` file). If that pattern is already used, a number will be appended to the name.
...
![Figure 1: Visual representation of subsetting a gen_tibble object](./img/update_backingfile.jpg){width=630px}
...
To fix this, we need to update the backing files to reflect the new set of individuals. This will create a new set of backing files on disk that match the individuals in our `gen_tibble` object. It is important that we use `<-` to ensure that the names of the newly updated backingfiles are stored in the `gen_tibble` object:
```{r}
new_example_gt <- gt_update_backingfile(new_example_gt, backingfile = tempfile())
```
...
Lets create another `gen`_tibble`, where loci are not ordered:
...
```{r}
reorder_test_gt_again <- gt_order_loci(reorder_test_gt, ignore_genetic_dist = FALSE)
```

And, again, we can check that the loci are ordered with respect to genetic distance:

```{r}
is_loci_table_ordered(new_example_gt, ignore_genetic_dist = FALSE)
```
