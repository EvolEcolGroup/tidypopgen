...
```{r}
anole_pca <- anole_gt %>% gt_pca_partialSVD(k=30)
#, fun_scaling = big_scale(scale=FALSE))
```
...
And plot the data in PCA space:
```{r}
library(ggplot2)
...
the functionality is not yet available in `tidypopgen` (at some point, we probably will remove this,
