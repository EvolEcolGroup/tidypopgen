...
It is also possible to inspect which loci contibute the most to a given component:
...
For more complect plots, we can augment the loci table with the loadings using `augment_loci()`:
```{r}
...
DAPC can be either used to test a a-prioriy hypothesis, or we can use the data to
...
from `adegenet`. This means that funcitons written to work on `dapc` objects
...
Then we can tidy our q matrix, and attach the population and id data from our original gen_tibble:

```{r}
tidy_q <- tidy_q(q_mat, anole_gt)
```


And we can plot the data using ggplot

```{r}
tidy_q %>% 
  ggplot() +
  geom_col(aes(x = id, y = q, fill = pop)) +
  labs(fill = "Population", x = "Individual", y = "Ancestry Proportion") +
  theme_minimal() +
...
To clean up the plot, we can order our individuals based on their population of origin and proportion derived from the q matrix:

```{r}

dominant_q <- tidy_q %>%
  group_by(id) %>%
  summarize(dominant_pop = pop[which.max(q)], dominant_q = max(q), .groups = 'drop')
