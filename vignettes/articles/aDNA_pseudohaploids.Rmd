...
```
## Warning in tidy.data.frame(ho_modern_pca): Data frame tidiers are deprecated and will be removed in an upcoming
## release of broom.
```
...
```
## Error in `vec_math()`:
## ! `vec_math.vctrs_bigSNP()` not implemented.
```
...
```
## Error: object 'variance' not found
```
...
```
## 
## gen_tibble saved to /tmp/Rtmp8nCm7A/gt_merged_fdbf3ed5f9.gt
```

```
## using bigSNP file: /tmp/Rtmp8nCm7A/gt_merged_fdbf3ed5f9.rds
```

```
## with backing file: /tmp/Rtmp8nCm7A/gt_merged_fdbf3ed5f9.bk
```
...
```
## gt_load('/tmp/Rtmp8nCm7A/gt_merged_fdbf3ed5f9.gt')
```
...
![plot of chunk pairwise_fst](./figure/aDNA_pseudohaploids/pairwise_fst-1.png)
