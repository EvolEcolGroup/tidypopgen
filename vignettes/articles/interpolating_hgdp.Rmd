...
We can store heterozygosity for each individual in our gen_tibble object by creating a column with `mutate()` and using the function `indiv_het_obs()` to calculate observed heterozygosity for each individual. 
...
Then we can create a new dataframe with the mean heterozygosity for each population
...
ggplot() +
  ...
  scale_fill_viridis_c( name = "Interpolated Heterozygosity", alpha = 0.8, na.value = NA ) +
  scale_color_viridis_c(name = "Observed", alpha = 0.8) +
  theme_minimal()
```

![plot of chunk interpolated_ggplot_map](./figure/interpolating_hgdp/interpolated_ggplot_map-1.png)
