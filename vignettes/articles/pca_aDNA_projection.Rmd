...
The example uses the data from Lazaridis et al. (2016), and replicates the method of the package `smartsnp`. This vignette follows a similar workflow, but in contrast to working directly from PACKEDANCESTRYMAP format files, we will load the same data into `gen_tibble` objects.
...
Here, we use the "least_squares" argument to generate a least squares projection that will be comparable to the method of `smartsnp`.
...
![plot of chunk pca_projected_aDNA](./figure/pca_aDNA_projection/pca_projected_aDNA-1.png)
