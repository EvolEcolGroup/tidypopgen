...
rmd_orig <- append(rmd_orig,
  values = c(
    ...
    "with a `r benchmarkme::get_cpu()` CPU",
    "and `r as.character(print(benchmarkme::get_ram(), unit_system = 'iec'))`",
...
rmd_laptop <- rmd_laptop[
  grep("# Running the benchmark on a laptop", rmd_laptop):length(rmd_laptop)
]
