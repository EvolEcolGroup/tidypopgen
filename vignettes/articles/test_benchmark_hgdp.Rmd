...
and `r print(benchmarkme::get_ram(), unit_system = "iec")` of RAM. However, we will limit the number of
...
tic()
pairwise_fsts <- grouped_hgdp %>% pairwise_pop_fst(n_cores = 3)
toc()
```
...
```{r echo=FALSE}
chunk_order <- knitr::all_labels() # Get labels in order
