...
tidypopgen has two key functions to examine the quality of data across loci and across individuals: `loci_qc_report` and `indiv_qc_report`. This vignette uses a simulated data set to illustrate these methods of data cleaning.

# Read data into gen_tibble format

```{r}
library(tidypopgen)
data <- gen_tibble(system.file("extdata/related/families.bed", package="tidypopgen"), quiet = TRUE, backingfile = tempfile(), valid_alleles = c("1","2"))
```
...
Here we can see there are many monomorphic SNPs in the data set. Let's filter out loci with a minor allele frequency lower than 2%, by using `select_loci_if`. Here, we select all SNPs with a MAF greater than 2%. This operation is equivalent to plink --maf 0.02.
...
We can see here that most SNPs have low missingness, under our 5% threshold, some do, however, have missingness up to around 10%. To remove these SNPs, we can again use `select_loci_if`.
...
Finally, if we may want to remove SNPs that show significant deviation from Hardy-Weinberg equilibrium, if our study design requires. To visualise SNPs with significant p-values in the Hardy-Weinberg exact test, we can again call autoplot:

```{r}
autoplot(loci_report,type = "significant hwe", p_val = 0.01)
```

Few SNPs in our data have significant HWE p-values, however there may be circumstances where we would want to cut out the most extreme cases, if these data were real, these cases could indicate genotyping errors.
...
The output of `loci_qc_report` supplies observed heterozygosity per individual, and rate of missingness per individual as standard.
...
And if we wanted to remove outliers with particularly high or low heterozygosity, we can again do so by using `filter`. Here we remove observations that lie more than 3 standard deviations from the mean.
...
Next, we can look at relatedness within our sample. If the parameter `cutoff` is provided to indiv_qc_report, then the report also calculates KING coefficient of relatedness of the sample. This threshold is used to create an output of the largest possible group with no related individuals, and the third boolean column `to_keep` recommends which individuals to remove to achieve this set.

```{r}
individual_report <- indiv_qc_report(data, cutoff = 0.177)
summary(individual_report)
```
...
`loci_ld_clump` then provides an output list of SNPs to keep in our data set. We can then use this list to create a pruned version of our data:

```{r}
ld_data <- imputed_data %>% select_loci(all_of(to_keep_LD)) 
```
