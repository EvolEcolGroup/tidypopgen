...
```{r}
example_gt %>%
  select_loci_if(loci_chromosomes(genotypes) == 2 &
...
We recommend using `reframe` to apply functions across groups. To use `reframe` correctly,
be sure to invoke the `pick` function:
```{r}
example_gt %>%
  group_by(population) %>%
...
This is important because dplyr functions `reframe`and `summarise` may apply
functions to all inidividuals unless the data in the current group are explicitly
selected.
...
tidy tibble, with one row per locus per population. However, using the `type`
