...
```{r}
lobsters <- gen_tibble(
  x = "../inst/extdata/lobster/lobster.bed",
...
individuals and columns are loci:

```{r}
lobsters %>% show_genotypes()
```
...
Then we can run a PCA. Here, we will use the `gt_pca_partialSVD` function:
...
within our dataset. For this, we can use ggplot2.
...
And then adding labels to the plot:
