...
```{r}
example_gt %>% group_by(population) %>% indiv_het_obs()
```
...
```{r}
example_clusters <- gt_cluster_pca(example_pca, n_pca = 165)
```

This object retains information about a large number of possible *k*. We can choose
the best k with `gt_cluster_pca_choose_n()` (we will use the defaults, but there
are multiple statistics and criteria that we can use to choose *n*):
```{r}
example_clusters <- gt_cluster_pca_best_k(example_clusters)
```
