# Working with aDNA pseuodhaploid samples

The following article shows how to handle ancient DNA in tidypopgen,
...
package `smartsnp`. Finally, we will replicate the pairwise Fst analysis
from Lazaridis et al. (2016).

First, we can download the data from the Reich lab website into a
temporary directory, and unzip the file.
...
Now that our data are downloaded, we can use `tidypopgen` to read the
...
Due to the damage associated with ancient DNA samples, genotype datasets
of ancient individuals often include pseudohaploid data. Instead of
diploid genotypes, low coverage samples are represented by a sampling
one allele per site. tidypopgen handles gen_tibbles containing
...
Once the ploidy has been assigned, pseudohaploid data is handled
...
and these functions will return an error.

For example, attempting to run `indiv_inbreeding` on pseudohaploid data
...
For our PCA analysis, we will be projecting ancient European
...
the `gen_tibble` object using the
...
To run a PCA on the modern Eurasian individuals, we need to impute any
missing genotypes. But as we have subset our `gen_tibble` object, first
...
We should also remove any monomorphic loci from our data, as these loci
are uninformative. Monomorphic markers must always be removed before a
PCA is computed using any of the `gt_pca` functions.

To do this, we use the `select_loci_if` function together with
...
We can then use `augment` to add the PCA coordinates for each individual
...
projection that will be comparable to the approach used in `smartpca`
(also implemented in the R package `smartsnp`).
...
The PCA and predicted individuals can then be visualized using `ggplot2`
by layering a geom of ancient individuals over modern individuals, using
...
![plot of chunk
pca_projected_aDNA](figure/aDNA_pseudohaploids/pca_projected_aDNA-1.png)

plot of chunk pca_projected_aDNA

## Pairwise Fst

Now let’s replicate the pairwise Fst analysis from Lazaridis et
al. (2016).
...
As we are calculating Fst between populations, we will remove any
...
We can then assign time ranges to the populations based on their names.
This will allow us to group populations by time period when creating our
plot:
...
After adding the time ranges, we can filter the pairwise Fst data to
...
Finally, we will calculate the median, minimum, and maximum values for
...
We can create our plot using ggplot2, adding the median, minimum, and
...
![plot of chunk
pairwise_fst](figure/aDNA_pseudohaploids/pairwise_fst-1.png)

plot of chunk pairwise_fst

## F statistics

To calculate F statistics, `tidypopgen` integrates with the R
`admixtools` package (a.k.a. ADMIXTOOLS 2). The function `gt_extract_f2`
allows users to calculate blocked f2 statistics from a `gen_tibble`
object, operating in the same way as the `extract_f2` function of
`admixtools`.
...
We could now use any of the functions from `admixtools` giving the
`outdir` with the f2 values.
