...
plot(
  pca.1$scores,
  type = "n",
  xlab = paste(round((pca.1$eig[1] / sum(pca.1$eig) * 100), 1), " %"),
  ylab = paste(round((pca.1$eig[2] / sum(pca.1$eig) * 100), 1), " %")
)
s.class(
  pca.1$scores,
  pop(aa.genlight.noOut),
  add.plot = T,
  xax = 1,
  yax = 2,
  cstar = F,
  cellipse = F,
  cpoint = 4,
  col = c("blue3", "red", "blue", "red3")
) # insert your PCA plotting code with better color scheme here ###
dev.off()
