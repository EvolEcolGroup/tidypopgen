...
pairwise_pop_fst_nei73 <- function(.x, by_locus = FALSE) {
  ...
  for (i_col in seq_len(ncol(pairwise_combn))) {
    ...
    Hs_pair[, i_col] <- (pop_counts_df[[pop1]]$het_exp +
      pop_counts_df[[pop2]]$het_exp) /
      2
    # Ht_pair is 1-p_bar^2 - q_bar^2
    Ht_pair[, i_col] <-
      1 -
      ((pop_counts_df[[pop1]]$sums_alt + pop_counts_df[[pop2]]$sums_alt) /
...
pairwise_pop_fst_nei83 <- function(.x, by_locus = FALSE) {
  ...
  for (i_col in seq_len(ncol(pairwise_combn))) {
    pop1 <- pairwise_combn[1, i_col]
    pop2 <- pairwise_combn[2, i_col]
    Hs_pair <- (pop_counts_df[[pop1]]$het_count +
      pop_counts_df[[pop2]]$het_count) /
...
if (diploid) {
  ...
  sHo <- (1 -
    t(apply(
      Ho,
      2,
      fun <- function(x) tapply(x, data[, 1], mean, na.rm = TRUE)
    )))
...
sp2 <- lapply(
  pop_counts_df,
  fun <- function(x) ((x$freq_alt)^2 + (x$freq_ref)^2)
)
...
sp2 <- lapply(
  pop_counts_df,
  fun <- function(x) ((x$freq_alt) + (x$freq_ref)) / 2
)
